...
function Game(eCanvas, ePlayer, eHex) {
    ...
    this.runeTypes = ["dam", "as", "cc", "cd", "range", "gold"];
}
...
Game.prototype.showPlayer = function() {
    ...
    var eRange = $('<p>Player Range: ' + this.player.radius + '</p>');
...
Board.prototype.createRunes = function() {
    for (var i = 0; i < this.hexes.length; i++ ) {
        ...
        if (this.hexes[i].rune !== "") {
            this.hexes[i].dam = this.runeStats[this.hexes[i].runeSet] * this.hexes[i].tier;
        }
    }
};
